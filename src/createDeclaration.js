...
export function createDeclaration(declaration, idx, mobile){
  ...
  return html`
      <template id="${(/** @type {any} */ (declaration)).tagName}" be-lazy>
...
function createSection(declaration, mobile){
  ...
  return html`
    <section itemscope id="${(/** @type {any} */(declaration)).tagName}">
        ...
        ${!(/** @type {any} */(declaration))?.members ? ''  : tablify((/** @type {any} */(declaration)).members.filter(/** @param x {any} */x => (x.kind === 'method') && (x.privacy !== 'private')) , 'Methods', 'https://cdn.jsdelivr.net/npm/custom-elements-manifest@1.0.0/schema.json#definitions/Method', mobile ,['kind'])}
    </section>
  `;
}